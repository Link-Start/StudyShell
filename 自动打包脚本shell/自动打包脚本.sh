...
export_path="/Users/liuxiaolong/Desktop/${scheme_name}_Package/$scheme_name-$DATE" #桌面
# 指定输出归档文件路径
...
if [ -d "$export_options_plist_path" ] ; then
    echo "${export_options_plist_path}文件存在，准备删除"
    rm -rf $export_options_plist_path

     if [[ -d "${export_options_plist_path}" ]]; then
        echo "${export_options_plist_path} 删除失败"
    else
        echo "${export_options_plist_path} 删除成功"
    fi
fi
